...
- Added configurable `KnowledgeHttpClient` using HTTPX.
- Added connect/read timeout, retry and backoff configuration.
- Added Knowledge Contract request construction in `KnowledgeHandler`.
- Added Knowledge response, evidence, trace and warning mapping into Execution Context.
- Added structured transport and service error mapping into Execution Contract errors.
- Connected the default `repeat_case_agent` runtime pipeline to the real Knowledge HTTP endpoint.
- Added WP-03 unit and runtime integration tests.

# WP-02 Business Agent Runtime

- Added AgentDefinition and AgentRegistry.
- Added runtime routing by agent ID.
- Added disabled-agent and unsupported-operation handling.
- Preserved failed-step traces.
- Added standard warning and artifact collection.
- Added WP-02 runtime tests and check script.
...
## V1.1 M1 I09 - Business Agent Contract Implementation

- Implemented QUALITY_AGENT_CONTRACT V1.0 version validation.
- Added BusinessAgentAdapter as the frozen in-process Business Agent integration boundary.
- Added request, response and error mapping modules.
- Added contract, adapter and runtime integration tests.
- Replaced placeholder checks with executable run_i09_check.py validation.
- Refreshed README as the current project entry point.

## I10 First Integration

- Verified the frozen Business Agent adapter with the real Business Agent KnowledgeClient.
- Added deterministic retrieval assets for first integration verification.
- Confirmed result, evidence, trace and error propagation.

## V1.1 M1 I11 - Project Cleanup

- Archived historical delivery documents under `docs/archive/delivery/`.
- Archived historical implementation status documents under `docs/archive/status/`.
- Archived historical governance and version documents under `docs/archive/governance/`.
- Archived I08/I09 check scripts under `docs/archive/scripts/`.
- Removed Python cache and operating-system temporary files.
- Refreshed README to describe the current I10 integration baseline and I11 cleanup state.
- Kept all runtime, compatibility, knowledge data, schema and test assets required by the current implementation.

## I12 - HTTP Transport

- Added FastAPI application entry.
- Added `GET /health`.
- Added `POST /v1/knowledge/query`.
- Reused `BusinessAgentAdapter` and existing Runtime without changing Contract semantics.
- Added HTTP status mapping for invalid request, missing service and runtime failures.
- Added HTTP tests and executable check script.
- Updated README with service startup instructions.

## WP-01 Execution Layer

- Added `QUALITY_AGENT_CONTRACT V1.1` Execution Contract models.
- Added `ExecutionRuntime.execute()` as the Business Agent public execution entry.
- Added an independently testable handler pipeline and execution trace collection.
- Added stable extension points for Context, Knowledge, Prompt, LLM and Result processing.
- Added `run_wp01_check.py` and six Execution Layer tests.
- Preserved the existing Knowledge HTTP and in-process transports without behavior changes.
- Full regression result: 156 passed.

## WP-04 Execution Pipeline

- Replaced placeholder PromptHandler with a knowledge-aware prompt builder.
- Added injectable LLMProvider contract and deterministic offline provider.
- Replaced placeholder LLMHandler with real provider invocation.
- Standardized ResultHandler output while preserving WP-01/WP-03 compatibility fields.
- Added WP-04 pipeline tests and check script.

## V1.1 M1 WP-05 - First Integration

- Added Business Agent HTTP API: `POST /v1/executions` and `GET /health`.
- Added executable Client -> Business Agent -> Knowledge -> REPEAT_CASE integration chain.
- Added strict Execution Contract boundary validation.
- Added Business Agent API CLI startup entry point.
- Added WP-05 end-to-end integration tests and delivery guide.
